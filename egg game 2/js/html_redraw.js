...
HTMLredraw.prototype.gameOver = function() {
  var msg = this.getMessage('ТЫ ПРОЕБАЛ МНОГО ГАВНА ТЕПЕРЬ ВОНЯЕТ');
...
HTMLredraw.prototype.gameWin = function() {
  var msg = this.getMessage('ТЫ ПАБЕДИЛ ТЫ ВАНЯ????');
...
HTMLredraw.prototype.getMessage = function(message) {
  var data = { h3: message, p: 'Нажми <b>R</b> чтобы начать сначала' };
